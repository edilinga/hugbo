...
public class UserBookingsController {
    ...
    public PagedResponse<BookingResponse> userBookings(
        ...
        if(!uid.equals(userId)) throw new IllegalArgumentException("forbidden"); //403
        ...
        if(!"asc".equalsIgnoreCase(dir) && !"desc".equalsIgnoreCase(dir)) throw new IllegalArgumentException("bad_dir");
        ...
        Page<Booking> pageResult = bookings.findByUserIdAndClassSessionStartAtBetween(userId, fromInst, toInst, pageable);
